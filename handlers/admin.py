...
All messages use professional English with Unicode icons — no emoji.
"""
...
async def owner_only(msg_or_cb, state: FSMContext | None = None) -> bool:
    ...
    if isinstance(msg_or_cb, CallbackQuery):
        await msg_or_cb.answer("Access denied. Owner only.", show_alert=True)
    else:
        await msg_or_cb.answer("Access denied. Owner only.")
    return False
...
async def _build_stats_text() -> str:
    ...
    return (
        ...
        f"▸ API Keys:          <b>{len(keys)}</b> total  |  "
        f"{valid} valid  |  {invalid} invalid  |  {active_keys} active\n"
        f"▸ Remaining Chars:   <b>{_fmt(total_chars)}</b>\n\n"
        f"▸ CPU Usage:         <b>{cpu:.1f}%</b>\n"
        f"▸ RAM:               <b>{mem.used // 1024 // 1024} MB</b> / "
        f"{mem.total // 1024 // 1024} MB ({mem.percent:.1f}%)\n"
...
async def cb_stats(cb: CallbackQuery) -> None:
    if not await owner_only(cb):
        return
    await cb.answer("Loading statistics…")
...
async def cb_key_info(cb: CallbackQuery) -> None:
    ...
    if sub:
        ...
        info = "Status:  Invalid or unreachable"
...
async def cb_key_add_start(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        ...
        "<i>Keys start with <code>sk_</code></i>",
...
async def receive_api_key(msg: Message, state: FSMContext) -> None:
    ...
    if not key:
        await msg.answer("Empty input. Operation cancelled.", reply_markup=kb.owner_main())
        return
    ok   = await db.add_api_key(key)
    keys = await db.get_all_api_keys()
    if ok:
        await msg.answer(
            f"<b>Key Added</b>\n\n<code>{_mask(key)}</code> has been saved.",
            reply_markup=kb.apikeys_menu(keys),
        )
...
async def cb_key_del_confirm(cb: CallbackQuery) -> None:
    ...
    key_id = int(cb.data.split(":")[2])
    await cb.message.edit_text(
        "<b>Confirm Removal</b>\n\nThis will permanently delete the API key. Proceed?",
...
async def cb_key_del_ok(cb: CallbackQuery) -> None:
    ...
    keys = await db.get_all_api_keys()
    await cb.message.edit_text(
        f"<b>Key #{key_id} Removed</b>\n\n{len(keys)} key(s) remaining.",
        reply_markup=kb.apikeys_menu(keys),
    )
...
async def cb_logs_menu(cb: CallbackQuery) -> None:
    if not await owner_only(cb):
        return
    await cb.message.edit_text(
        "<b>System Logs</b>\n\nSelect a time range and log level:",
...
async def cb_logs_view(cb: CallbackQuery) -> None:
    ...
    if not logs:
        text = f"<b>System Logs</b>  —  Last {minutes} min\n\nNo entries found."
    else:
        lvl_label = level or "ALL"
        lines     = [f"<b>System Logs</b>  —  Last {minutes} min  [{lvl_label}]  {len(logs)} entries\n"]
        for log in logs[:30]:
            ts   = log["created_at"][11:19]
            lvl  = log["level"][:4]
            icon = {"ERRO": "[ERR]", "WARN": "[WRN]", "INFO": "[INF]", "DEBU": "[DBG]"}.get(lvl, "[   ]")
            msg  = html.escape(log["message"][:120])
            lines.append(f"<code>{ts}</code> {icon} {msg}")
        if len(logs) > 30:
            lines.append(f"\n<i>… {len(logs) - 30} additional entries truncated.</i>")
        text = "\n".join(lines)
...
async def cb_users_list(cb: CallbackQuery) -> None:
    ...
    await cb.message.edit_text(
        "\n".join(lines) or "No users registered yet.",
...
async def cmd_userbio(msg: Message) -> None:
    ...
    text = (
        ...
        f"▸ TTS Count:   {user['tts_count']}\n"
        f"▸ STT Count:   {user['stt_count']}\n"
...
async def cb_ban_user(cb: CallbackQuery) -> None:
    ...
    await cb.answer("User has been banned.", show_alert=True)
    user = await db.get_user(uid)
    if user:
...
async def cb_unban_user(cb: CallbackQuery) -> None:
    ...
    await cb.answer("User has been unbanned.", show_alert=True)
    user = await db.get_user(uid)
    if user:
...
async def cb_broadcast_start(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        ...
        "Supported types: text, photo, video, audio, document, sticker.\n\n"
        "<i>You may also forward any message directly to the bot.</i>",
...
async def receive_broadcast_msg(msg: Message, state: FSMContext) -> None:
    ...
    _, total = await db.get_all_users(0, 1)
    await msg.answer(
        f"<b>Broadcast Preview</b>\n\n{preview}\n\n"
        f"▸ Recipients: <b>{total}</b> users\n\nConfirm and send?",
...
async def cb_settings(cb: CallbackQuery) -> None:
    if not await owner_only(cb):
        return
    bot_online = await db.get_setting("maintenance", "0") != "1"
    await cb.message.edit_text(
        "<b>Bot Settings</b>\n\n"
